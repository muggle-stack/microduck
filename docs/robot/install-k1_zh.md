...
**2026-09-06** 检查时，官方 `pollen-robotics/microduck:main` 仍为本分支已包含的 `bc41fb5`，
当时安装指南无需 rebase。以后要检查，应 fetch `upstream` 并审阅变更；
仅在工作区干净时 rebase，改写共享分支历史前协调，并重新执行 K1 回归。
不要仅为对齐名称强推。
...
已测 IMX219 的画质和运行稳定性获用户确认；单观众 WebRTC 图传与同源 ORT/EP 检测已可用，
见[当前适配进度](../project/spacemit-k1-adaptation-zh.md)。
