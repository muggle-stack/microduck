...
后续浏览器图传和同源推理的使用与验收见 [K1 WebRTC](k1-webrtc.md)。
...
历史实验及精度边界见 [模型实验](k1-duck-quantization.md)。
...
用户已在 Mac 浏览器实际运行 [WebRTC 与同源 ORT / EP 推理](k1-webrtc.md)，
...
[WebRTC 验证与用户确认](k1-webrtc.md)，不是将 `camera-check` 成功外推为图传完成。
