...
曝光/白平衡及色彩链路的后续专项回归依据。本轮没有通过临时滤镜或修改检测精度掩盖图像问题。
...
用户已在 Mac 浏览器实际运行 [WebRTC 与同源 ORT / EP 推理](k1-webrtc_zh.md)，
确认图传可用、推理正常，并进一步确认 **IMX219 的运行稳定性和图传质量可接受**。
据此，指定 MUSE-Pi-Pro / CSI3 IMX219 的摄像头 / ISP 功能记为用户实测通过，
不再将当前图像质量和基本运行稳定性列为未完成项。

用户反馈未附使用时长、帧统计或光照条件记录，不能换算为若干小时无故障指标，
也不代表早期偏青/过曝现象、vendor 告警或全部异常场景已经定位消除。
保留前述开发数据；带标注的检测精度、量化长稳和整机并发仍单独验收。
...
[WebRTC 验证与用户确认](k1-webrtc_zh.md)，不是将 `camera-check` 成功外推为图传完成。
另有显式启用的 SDK [`camera-rtsp` 工具](k1-rtsp_zh.md)，将 ISP 的 NV12 DMA-BUF 接入
K1 硬编码和 RTSP/TCP，可在电脑播放器实时预览；它不加载检测器，也不等同于 WebRTC。
vendor ISP 管理 AE/AWB；SDK 不复用 Rockchip 手动曝光控制。当前画质和运行稳定性已获用户认可；
