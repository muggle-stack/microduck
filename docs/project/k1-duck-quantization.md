...
- XSlim INT8 经最终输出修正后，K1 4 核平均 **21.57 ms**，2 核 **32.77 ms**。
  相对同核数原模型 CPU 基线，分别约 **22.3 倍、21.7 倍**。
- 速度不等于精度通过：12 帧诊断样本中，原模型输出 11 个框，最终 INT8 模型输出
  8 个对应框。无人工标注，不能把这些数字称作 mAP、真实召回率或准确率。
- 量化产物只用于 opt-in 实验，**没有替换 SDK 的默认模型，也没有接入 Rust EP 后端**。
  如果先推进 SDK 集成，浮点模型经 opset 转换后的 EP 路径也是候选：4 核约
...
- 2 核固定 `taskset -c 0-1`，4 核固定 `taskset -c 0-3`；ORT intra-op 与 EP 线程数
  分别显式设为 2/4，inter-op=1，图优化为 ALL。EP affinity 与 cpuset 一致。
...
- 最终 INT8 / EP / 4 核另测 120 次：平均 **21.70 ms**，P95 **23.68 ms**。
...
K1 原生编译与 4 核测试（原模型 CPU 对照时更换模型路径、去掉 env，并将 `ep` 换为 `cpu`）：
