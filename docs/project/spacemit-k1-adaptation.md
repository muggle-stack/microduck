...
IMX219 (`ba2776c`), RTSP (`e73a4fa`), WebRTC (`229c7f3`), model delivery and user-acceptance documentation.
...
The aligned upstream baseline is `bc41fb5`; this does not imply a fresh check of today's upstream.
...
This summarizes existing acceptance results, not a new whole-robot test.
...
- **Five verified** within their stated software/board scope, including user confirmation of current IMX219 quality and stability.
...
| 5 | Camera / ISP | Verified | USB selected-eye and optional MPP/V2D verified; IMX219 K1 CSI/ISP → SDK → ORT/EP works, standalone capture about 26.7 fps and detection meets 2 Hz. User accepts current IMX219 quality/stability; stress, abnormal recovery and other modules remain separate. |
| 6 | H.264 / RTSP / WebRTC streaming | Partial | RTSP retained; IMX219 → K1 H.264 → official console video, DataChannel, same-source ORT/EP notifications and reconnect/shutdown verified. User confirms Mac video/inference and current quality/stability. Audio/video, adaptive bitrate, multiple viewers, USB streaming and whole-robot load remain. |
...
**Current USB requirements are selected-eye use and are implemented.**
...
not every module or image. Concurrent dual-eye inference, stereo depth and calibration are outside this phase.
...
On 2026-09-07 the user confirmed **acceptable IMX219 stability and streaming quality**.
The specified board/scenario's camera/ISP functionality is therefore verified.
No duration, frame statistics or lighting matrix accompanied that feedback;
it is not a specified failure-free-hours, stress, abnormal-recovery or other-module acceptance result.
...
Subsequently the user confirmed working video/inference in a Mac browser and accepted current IMX219 quality/stability.
This is user functional confirmation, not labelled accuracy or a new four-CPU benchmark.
...
These are scope and completion criteria, not promised dates.
Independent tasks need not wait for one another; hardware tasks proceed when peripherals arrive.
Tested USB selected-eye and IMX219 are both available development inputs.
...
Before delivery, **whole-robot qualification** must cover simultaneous motion, camera, vision, audio and streaming:
...
Current IMX219 quality/stability are accepted; future regression should record duration/error statistics,
different-lighting exposure/white balance, network/camera abnormal recovery and existing vendor warnings.
These extend a verified path; basic IMX219 availability is no longer a blocker.
...
The original RKNN implementation and motion-control core were not removed; Rockchip camera and AIC3104 defaults remain.
K1 support is selected through config/optional backends, but also changes shared configuration, frame acquisition,
ORT wrapping and ALSA device-name handling. It is not an entirely untouched original codebase.
...
Future updates must follow implementation and evidence.
Unconnected hardware, compilation-only results or isolated demos are not prematurely labelled whole-robot completion.
