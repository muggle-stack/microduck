...
- 单观众 IMX219 视频与同源检测链路已验证可用。早期天花板场景的偏色/高光过曝记录保留，
  不同光照下的 PQ 和曝光仍需专项回归，测试未使用额外滤镜。
...
Mac 浏览器可以接收单观众 IMX219 视频和同源 ORT / EP 检测通知。
上方测试记录给出了实际解码帧数、检测通知数量，以及重连和关闭结果。

上述数据对应原有 4 核短测条件；端到端时延、带标注检测精度和量化长稳尚未测量。
音视频、多观众、网络/相机异常恢复与整机并发仍需专项测试。
