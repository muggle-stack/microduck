...
- The user accepted current IMX219 streaming quality and running stability.
  ...
  The SDK does not conceal it with filters.
...
### Subsequent user acceptance: 2026-09-07

The user independently started and used the Mac browser workflow, confirmed working video/inference,
and then confirmed **acceptable IMX219 stability and streaming quality**.
Single-viewer IMX219 video with same-source detection is available for development;
user-visible image quality is no longer an unaccepted blocker.

This feedback includes no duration, frame statistics or labelled set, so no new frame-rate,
latency, accuracy or endurance figures are added.
The original four-CPU short-test conditions remain unchanged.
Audio/video, multiple viewers, network/camera abnormal recovery and whole-robot concurrency still need dedicated acceptance.
